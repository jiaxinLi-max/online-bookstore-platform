<template>
  <div class="product-detail">
    ...
    <el-dialog
        v-model="isColumnDialogVisible"
        :title="selectedColumn.theme + ' 栏目下的所有书籍'"
        width="60%"
        class="column-dialog"
    >
    ...
    >
      <el-form ref="form" label-width="120px" class="product-form">
        ...
        <el-form-item label="所属栏目" prop="columnIds">
          <el-select v-model="editForm.columnIds" multiple placeholder="请选择商品所属的栏目" style="width: 100%;">
            <el-option
  ...
  setup() {
    ...
    const loadProductDetails = async (id: string) => {
      try {
        ...
        if (!Array.isArray(product.value.cover)) {
          product.value.cover = [];
        }
    ...
    const openEditDialog = () => {
      ...
      if (!Array.isArray(editForm.value.cover)) {
    ...
    const goToProductDetail = (id: number) => {
      ...
      if (id === Number(productId)) {
        ...
        router.push({ path: `/home/product/${id}` });
      }
    }
    ...
    const getStock = async () => {
      try {
        ...
        } else { ElMessage.error(response.data.msg || '获取库存失败'); }
    ...
    const handleCreateComment = async () => {
      ...
      } catch (error) { ElMessage.error('创建评价失败'); }
    };

    const viewAllComments = () => {
      router.push({ name: 'ProductComments', params: { productId: productId } });
    };
...
.column-tag.clickable {
  cursor: pointer;
  margin-right: 8px;
  margin-bottom: 8px;
  transition: all 0.2s;
}
.column-tag.clickable:hover {
